...
TEST(MelSpectrogram, PrecomputationSeparateBuffer) {
    ...
    /// Compare mel spectrogram bins in separate buffers
    {
        float mel_centre_freq_float_buffer[MEL_CENTRE_FREQ_FLOAT_BUFFER_LENGTH] = {};
        uint16_t mel_centre_freq_next_bin_buffer[MEL_CENTRE_FREQ_NEXT_BIN_BUFFER_LENGTH] = {};
        uint16_t mel_centre_freq_prev_bin_buffer[MEL_CENTRE_FREQ_PREV_BIN_BUFFER_LENGTH] = {};
        float mel_freq_weights_buffer[MEL_FREQ_WEIGHTS_BUFFER_LENGTH] = {};

        assert(sizeof(mel_centre_freq_float_buffer) == MEL_CENTRE_FREQ_FLOAT_BUFFER_SIZE);
        assert(sizeof(mel_centre_freq_next_bin_buffer) == MEL_CENTRE_FREQ_NEXT_BIN_BUFFER_SIZE);
        assert(sizeof(mel_centre_freq_prev_bin_buffer) == MEL_CENTRE_FREQ_PREV_BIN_BUFFER_SIZE);
        assert(sizeof(mel_freq_weights_buffer) == MEL_FREQ_WEIGHTS_BUFFER_SIZE);
